...
- Stage 5B's third independent code review passed: ADMIN-only keyword/vector/hybrid retrieval, RRF, optional fallback-safe reranking and retrieval-run audit are complete. Real Qdrant smoke still requires a Docker/Qdrant-capable environment; until that retest, do not merge master, create a `rag-stage-5b-passed` tag, or begin Stage 6. RAG generation and Agentic RAG remain out of scope.
...
Stage 5A passed Claude's final independent review. The approved scope was knowledge-vector indexing lifecycle only: stable embedding input, profile-isolated Qdrant collections, verified upserts, atomic active-run switching, cleanup tracking, and delete synchronization. Stage 5B then passed its third independent code review, adding ADMIN-only retrieval and reranking; real Qdrant smoke remains an environment-gated retest, and RAG or agents do not exist.

The durable task workflow is now documented in `docs/CONTEXT_INDEX.md`, `docs/PROJECT_STATUS.md`, `docs/CURRENT_TASK.md`, `docs/DECISIONS.md`, `docs/tasks/`, and `docs/reviews/`. Read these before relying on this historical summary. Stages 1–5A are accepted; Stage 5B code review passed but its real Qdrant smoke retest remains pending in a Docker/Qdrant-capable environment.
