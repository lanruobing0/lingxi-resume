...
- Bounded Agentic RAG：`POST/GET /api/job-applications/:id/agent-runs`、`GET /api/agent-runs/:id`、`GET /api/agent-runs/:id/steps`
...
- `tests/agent-ui.unit.mjs`、`tests/agent-ui.render.mjs`：AgentRun 创建请求、历史恢复、六类 action、六种状态、步骤时间线、安全来源、四类 finalResult、敏感字段过滤和 loading/empty/error。
