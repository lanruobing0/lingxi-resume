...
阶段 5B 第三次独立代码验收已通过：ADMIN 可使用关键词、向量或混合检索实验室，检索只返回本地可验证的当前有效 Chunk。RRF 是默认融合策略；Reranker 默认关闭，失败回退 RRF。真实 Qdrant smoke 仍待具备 Docker/Qdrant 的环境复验；复验完成前不得合并 master、创建 `rag-stage-5b-passed` 标签或开始阶段 6。`corepack pnpm test:qdrant-retrieval` 是独立真实 Qdrant 检索 smoke。
